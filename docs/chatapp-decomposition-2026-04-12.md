...
- Вынесена модалка создания чатов в `NewChatModal` (поиск пользователей + создание группы/канала), чтобы убрать крупный условный JSX-блок из корневого компонента.
- Вынесена модалка информации о канале в `ChannelInfoModal` (публичная ссылка, переход к управлению и вложениям), чтобы разгрузить `ChatApp` от channel-info разметки.
